...
class SurvivalGame:
    ...
    def _create_bar(self, value):
        """Create a visual bar for displaying stats"""
        filled = int(value / 10)
        empty = 10 - filled
        return f"[{'█' * filled}{'░' * empty}] {int(value)}/100"
    ...
    def display_status(self):
        ...
        print("-"*60)
        print(f"❤️  Health:  {self._create_bar(self.health)}")
        print(f"🍗 Hunger:  {self._create_bar(self.hunger)}")
        print(f"💧 Thirst:  {self._create_bar(self.thirst)}")
        print(f"⚡ Energy:  {self._create_bar(self.energy)}")
        print("-"*60)
    ...
    def update_environment(self):
        """Update weather and temperature based on current weather"""
        ...
        temp_change = weather_effects.get(self.weather, 0) + random.randint(-2, 2)
        self.temperature += temp_change
        self.temperature = max(-40, min(50, self.temperature))

    def apply_temperature_damage(self):
        """Apply health damage based on temperature and shelter level"""
    ...
    def show_menu(self):
        """Display action menu and return player choice"""
        print("\n🎮 ACTIONS:")
    ...
    def search_resources(self):
        """Search current location for wood and stones"""
    ...
    def consume_resources(self):
        """Consume food and water when needed"""
        if self.hunger < 50 and self.inventory["food"] > 0:
            ...
            print(f"🍽️  Ate {food_amount} food (+{food_amount * 4} hunger)")
    ...
    def check_survival_status(self):
        """Check if player is alive and apply daily effects"""
        # Degrade stats naturally throughout the day
        ...
        # Consume resources if critical
        if self.hunger < 30 or self.thirst < 30:
            self.consume_resources()

        # Convert negative stats to health damage
        if self.hunger < 0:
    ...
    def end_game(self):
        ...
        print(f"Fire Level: {int(self.fire_level)}/5")
        print("="*60 + "\n")
